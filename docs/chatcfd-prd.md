...
- [17. 实施路线](#17-实施路线)
- [18. 🆕 跨会话记忆系统（Mempalace 集成）](#18-跨会话记忆系统mempalace-集成)
- [19. 测试用例](#19-测试用例)
...
│                                │                              │
│  ┌──────────────────────────────────────────────────────┐    │
│  │  MCPClientPool（多 server 路由）                       │    │
│  │  ┌─────────────────┐  ┌────────────────────────────┐ │    │
│  │  │ SSE Client       │  │ stdio Client               │ │    │
│  │  │ → post_service   │  │ → mempalace                │ │    │
│  │  │ 6 tools (全暴露)  │  │ 4/21 tools (过滤后暴露)    │ │    │
│  │  └────────┬────────┘  └─────────────┬──────────────┘ │    │
│  └───────────┼─────────────────────────┼────────────────┘    │
│              │                         │                      │
│  Harness │ Skill │ Prompt              │                      │
│  主 Agent │ 子 Agent 池               │                      │
│  Insight Log │ AgentSession            │                      │
│    (memory_wing / loaded_file_path)    │                      │
└──────────────┬─────────────────────────┼─────────────────────┘
               │ MCP(SSE)               │ MCP(stdio)
               │ (summary给LLM)         │ (记忆读写)
               │                        │
┌──────────────▼─────────────┐  ┌───────▼──────────────────────┐
│  后处理服务（FastAPI+FastMCP）│  │  Mempalace 记忆服务           │
│                             │  │                               │
│  ┌───────────┐ ┌──────────┐│  │  ┌───────────────────────┐   │
│  │MCP 端点    │ │HTTP API  ││  │  │ 暴露给 LLM (4 tools)   │   │
│  │6 tools     │ │mesh/     ││  │  │ search / add_drawer    │   │
│  │给 LLM      │ │scalar/   ││  │  │ kg_query / kg_add      │   │
│  └─────┬─────┘ │file/     ││  │  ├───────────────────────┤   │
│        │       │upload    ││  │  │ 代码层自动调 (17 tools) │   │
│        │       └────┬─────┘│  │  │ check_dup / invalidate │   │
│        └──────┬─────┘      │  │  │ diary_* / traverse ... │   │
│               ▼            │  │  └───────────┬───────────┘   │
│  ┌──────────────────────┐  │  │              ▼               │
│  │  PostEngine 计算引擎   │  │  │  ┌───────────────────────┐  │
│  │  Session Manager      │  │  │  │ ChromaDB (语义搜索)     │  │
│  │  PostData(零拷贝)     │  │  │  │ bge-small-zh embedding │  │
│  │  算法注册表 │ VTK 层  │  │  │  ├───────────────────────┤  │
│  │  分析存档(Archive)    │  │  │  │ SQLite (知识图谱)       │  │
│  └──────────────────────┘  │  │  │ 时序三元组(用户偏好)    │  │
│                             │  │  └───────────────────────┘  │
└─────────────────────────────┘  └──────────────────────────────┘
         ▲                                    ▲
         │                                    │
    精确计算结果                          模糊偏好/发现/经验
    结构化存储(.chatcfd/)                语义化存储(~/.mempalace/)
    md5 校验,跟文件走                    跨项目可搜索
    用户主动"保存结果"                   LLM 自动沉淀
```

要点：
- **双 MCP 架构**：MCPClientPool 同时管理 SSE（post_service）和 stdio（mempalace）两个 MCP server
- **工具过滤**：Mempalace 21 个工具中只暴露 4 个给 LLM（search/add_drawer/kg_query/kg_add），其余代码层自动调用，合计 10 个工具
- **双存储分工**：精确计算结果 → Analysis Archive（结构化、按文件索引、md5 校验）；模糊偏好和工程发现 → Mempalace（语义化、跨项目搜索）
- **Embedding 替换**：ChromaDB 使用 bge-small-zh-v1.5（96MB 中文模型），替代默认的英文 all-MiniLM-L6-v2
- **记忆注入时机**：对话开始注入全局偏好；loadFile 后按 wing 检索项目记忆；不盲目注入
- 详见 [Mempalace 集成方案](plans/2026-04-08-mempalace-integration.md) 和 [待办事项](plans/2026-04-08-mempalace-todo.md)
...
| method | 功能 | 依赖 | 状态 |
|--------|------|------|------|
| `force_moment` | 力/力矩积分，升力/阻力系数 | VTK C++（ForceMomentIntegtal） | Phase 1 ✅ |
| `velocity_gradient` | 速度梯度/涡量/Cp/马赫数 | VTK C++（CalculateVelocityGradient） | Phase 1 ✅ |
| `statistics` | 标量统计 min/max/mean/std | numpy | Phase 1 ✅ |
| `compare` | 两区域/两文件标量对比 | numpy | Phase 1 ✅ |
| `slice` | 切片截面 | VTK（vtkCutter / SliceFilter） | 🆕 提前实现 ✅ |
| `clip` | 裁剪（保留一半几何体） | VTK（vtkClipDataSet） | 🆕 新增 ✅ |
...
| 组件 | 职责 |
|------|------|
| `ChatPanel.vue` | 对话消息区，WebSocket 通信 |
| `MessageBubble.vue` | 单条消息渲染（用户/AI/工具调用） |
| `ArtifactPanel.vue` | Artifact 侧边栏（列表 + 展示区） |
| `ArtifactList.vue` | Artifact 列表管理 |
...
- [x] 数据层 PostData 薄封装实现（零拷贝 + writeable=False 保护）
- [x] 算法插件化目录 `algorithms/`，自动扫描加载
- [x] DEFAULTS 用 dict，统一返回格式 {type, summary, data, output_files}
- [x] 6 个 MCP 工具实现（loadFile / calculate / compare / exportData / listFiles / getMethodTemplate）
- [x] 现有 2 个算法（force_moment / velocity_gradient）迁移到插件
- [x] 新增 statistics 算法（min/max/mean，不依赖 VTK）
- [x] 会话缓存机制（SessionState + Session Manager）
- [x] MCP Prompt（Skill 工作流引导）— 🆕 重构为 ROLE/TOOLS/RULES 三层结构
- [x] AI Coding 确认机制（Harness 层拦截）
- [x] Web 前端（对话界面 + Artifact 侧边栏）
- [x] VTK.js 基础 3D 渲染（加载网格 + 标量着色 + 旋转/缩放/平移）
- [x] Artifact 展示：3D 视窗 + JSON 卡片 + 数据表格
...
- [ ] 子 Agent：完整报告生成 — 🆕 可接入 Mempalace diary
...
## 18. 🆕 跨会话记忆系统（Mempalace 集成）

> **新增（2026-04-08）**：通过集成 Mempalace 实现跨会话记忆能力，让 AI 助手记住用户的分析偏好、工程发现和历史结论。

### 18.1 架构

Agent 通过 MCPClientPool 同时连接两个 MCP server：

| MCP Server | 协议 | 用途 |
|------------|------|------|
| post_service | SSE (HTTP) | CFD 后处理计算 |
| mempalace | stdio (subprocess) | 跨会话记忆存取 |

### 18.2 工具过滤策略

Mempalace 共 19+ 个工具，仅暴露 4 个给 LLM，其余代码层自动调用：

| 暴露给 LLM | 用途 |
|-----------|------|
| `mempalace_search` | 语义搜索历史记忆 |
| `mempalace_add_drawer` | 存储分析结论/发现 |
| `mempalace_kg_query` | 查询用户偏好（参照系/来流条件） |
| `mempalace_kg_add` | 记录用户偏好变更 |

合计暴露给 LLM 的工具：6（post_service）+ 4（mempalace）= **10 个**。

### 18.3 记忆注入时机

| 时机 | 注入内容 | 实现方式 |
|------|---------|---------|
| 对话开始（首条消息） | 全局用户偏好 | 代码层调用 `mempalace_kg_query(entity="user")` |
| loadFile 成功后 | 同项目的历史分析记忆 | 代码层调用 `mempalace_search(wing=推断的wing)` |
| 用户说"上次/之前" | 按查询语义检索 | LLM 主动调用 `mempalace_search` |
| 分析得出结论 | LLM 判断是否存入 | LLM 主动调用 `mempalace_add_drawer` |
| 对话结束（WebSocket 断开） | 自动保存工具 summary | 代码层 `_try_extract_memories()` |
...
推断规则：跳过 data/cgns/results 等通用目录名，取第一个有意义的上级目录。

**Room**（分析主题）— LLM 从预定义列表选择：

| room | 存什么 |
|------|-------|
| results | 数值结论（CL=0.45, max_pressure=120000） |
| parameters | 用户常用参数（参考面积、来流条件） |
| visualization | 可视化偏好（切面位置、等值面阈值） |
| findings | 工程发现（翼尖分离、激波位置） |
| workflow | 工作流模式（先力矩再涡量） |

### 18.5 与 Analysis Archive 的分工

| | Analysis Archive（§14） | Mempalace |
|---|---|---|
| 存什么 | 精确计算结果 + 参数 | 模糊偏好 / 工程发现 / 工作流模式 |
| 索引方式 | 按文件路径 + md5 | 按 wing/room + 语义搜索 |
| 触发 | 用户主动说"保存" | LLM 判断 + 代码层自动 |
| 存储位置 | `.chatcfd/`（跟着数据文件走） | `~/.mempalace/`（全局） |
| 跨文件搜索 | 不支持 | 支持 |

### 18.6 自动偏好覆盖

用户偏好存储在知识图谱（SQLite 时序三元组）。LLM 调用 `mempalace_kg_add` 时，代码层自动 invalidate 同 subject+predicate 的旧三元组：

```
旧: (user, prefers_ref_area, 1.0)    valid_to=2026-04-09  ← 自动失效
新: (user, prefers_ref_area, 0.5)    valid_from=2026-04-09
```

### 18.7 启用方式

`.env` 文件添加 `MEMPALACE_ENABLED=true`。默认关闭（false），不影响原有功能。

需要在同一 Python 环境安装 mempalace：`pip install -e <mempalace_path>`

详见 [Mempalace 集成方案](plans/2026-04-08-mempalace-integration.md)
...
### 19.3 Phase 2 补充测试（compare / Artifact / 统一返回格式）
